...
function render() {
    ...
    grid.innerHTML = meds.map(m => `
        <div class="med-card">
            <img src="${m.image}" alt="${m.name}" style="width:100px; height:100px; border-radius:10px; margin-bottom:10px;">
            <h3>${m.name}</h3>
            <p>${m.price} ريال</p>
...
function add(id) {
    ...
    document.getElementById('cartItems').innerHTML = cart.map(i => `<div style="padding:10px; border-bottom:1px solid #eee">${i.name}</div>`).join('');
}
function logout() {
